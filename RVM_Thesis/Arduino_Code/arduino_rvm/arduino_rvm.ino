...
void setup()
{
  ...
  servoSortTrash.attach(SERVO_360_PIN);
  servoLoadCell.attach(SERVO_160_PIN);
  servoSortTrash.write(0); // Stop position
  servoLoadCell.write(0);  // Center position
...
void processPlasticItem()
{
  ...
  if (weight > WEIGHT_THRESHOLD)
  {
    activateservoSortTrash();
  }
  else
  {
    activateservoLoadCell();
  }
...
void activateservoSortTrash()
{
  Serial.println("INFO: Activating Inlet servo (weight > 50g)");
  servoSortTrash.write(0);
  delay(3000);
  servoSortTrash.write(60);
  delay(1000);
  Serial.println("INFO: Inlet servo operation complete");
}

void activateservoLoadCell()
{
  Serial.println("INFO: Activating Load cell servo (weight <= 50g)");
  for (int pos = 0; pos <= 30; pos += 1)
  {
    servoLoadCell.write(pos);
    delay(50);
  }
  delay(1000);
  ...
  Serial.println("INFO: Load cell servo operation complete");
}
...
void activateDcMotor()
{
  ...
  delay(3000); // wait 3 seconds before stop
